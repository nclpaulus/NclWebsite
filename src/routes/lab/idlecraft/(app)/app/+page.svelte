			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				...
				<div class="flex items-center gap-2 p-3 rounded-lg border bg-card">
					...
					<div>
						...
						<div class="text-lg font-bold">{data.player.wood_per_sec}/s</div>
					</div>
				</div>
				...
				<div class="flex items-center gap-2 p-3 rounded-lg border bg-card">
					...
					<div>
						...
						<div class="text-lg font-bold">{data.player.ore_per_sec}/s</div>
					</div>
				</div>
			</div>
					...
					use:enhance={() => {
						...
						return async ({ result }) => {
							...
							if (result.type === 'success' && result.data?.success) {
								const syncData = result.data as { success: boolean; tick: any; player: PageDataInterface['player'] };
								lastSyncGold = syncData.player.gold;
								lastSyncTick = syncData.tick;
								// Mettre à jour les données de la page en préservant l'inventaire
								data = {
