<script lang="ts">
	...
	interface PageDataInterface {
		...
		inventory: Array<{ item_key: string; qty: number }>;
		tick: {
		...
		upgrades: Array<{
			...
			cost_wood: number;
			cost_ore: number;
			gold_per_sec_bonus: number;
			wood_per_sec_bonus: number;
			ore_per_sec_bonus: number;
			name: string;
			description: string;
			icon: string;
			owned: boolean;
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				...
				<div class="flex items-center gap-2 p-3 rounded-lg border bg-card">
					...
					<div>
						...
						<div class="text-lg font-bold">{(data.inventory.find((i) => i.item_key === 'wood')?.qty || 0).toLocaleString()}</div>
				...
				<div class="flex items-center gap-2 p-3 rounded-lg border bg-card">
					...
					<div>
						...
						<div class="text-lg font-bold">{(data.inventory.find((i) => i.item_key === 'ore')?.qty || 0).toLocaleString()}</div>
					...
					use:enhance={() => {
						...
						return async ({ result }) => {
							...
							if (result.type === 'success' && result.data?.success) {
								const syncData = result.data as { success: boolean; tick: any; player: PageDataInterface['player']; inventory: any[] };
								lastSyncGold = syncData.player.gold;
								lastSyncTick = syncData.tick;
						...
						<div class="flex items-start justify-between">
							<div class="flex-1">
								...
								<div class="text-xs text-muted-foreground mt-1">
									{#if upgrade.gold_per_sec_bonus > 0}
										+{upgrade.gold_per_sec_bonus} gold/s
									{/if}
									{#if upgrade.wood_per_sec_bonus > 0}
										+{upgrade.wood_per_sec_bonus} wood/s
									{/if}
									{#if upgrade.ore_per_sec_bonus > 0}
										+{upgrade.ore_per_sec_bonus} ore/s
									{/if}
								</div>
							</div>
							...
							{#if !upgrade.owned}
								...
								>
									...
									<Button 
										type="submit" 
										size="sm"
										disabled={data.player.gold < upgrade.cost_gold}
										variant={data.player.gold < upgrade.cost_gold ? "outline" : "default"}
									>
										{upgrade.cost_gold} <Coins class="w-3 h-3 ml-1" />
									</Button>
								</form>
							{/if}
						</div>
		...
		<CardContent class="p-6 space-y-3">
			...
			{:else}
				<div class="grid grid-cols-2 md:grid-cols-3 gap-2">
					{#each data.inventory as row, i}
						<div 
							class="rounded-lg border p-3 hover:bg-accent transition-colors"
							in:slide={{ duration: 300, delay: i * 50 }}
						>
							<div class="font-medium capitalize">{row.item_key.replace('_', ' ')}</div>
							<div class="text-2xl font-bold text-primary">{Number(row.qty).toLocaleString()}</div>
						</div>
					{/each}
				</div>
			{/if}
		...
		<CardContent class="p-6 space-y-3">
			...
			<div class="space-y-2">
				...
				<form 
					...
					action="?/resetAccount"
					onsubmit={(e) => {
						if (!confirm('Êtes-vous sûr de vouloir réinitialiser votre compte ? Toutes les upgrades et ressources seront perdues !')) {
							e.preventDefault();
						}
					}}
					use:enhance={() => {
